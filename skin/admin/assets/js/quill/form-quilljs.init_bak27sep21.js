(function() {
	...
	placeholder: 'Enter Project Statement here.',
	theme: 'snow',
	});
	...
	/*------------ Add Goal-----------*/
	var GoalAdd = function (quilljs_textarea_goaladd, options) {
	  this.quilljs_textarea_goaladd = quilljs_textarea_goaladd;
	  this.options = options;
	  this.container = document.querySelector(options.container);
	  quilljs_textarea_goaladd.on('text-change', this.update.bind(this));
	  this.update(); // Account for initial contents
	};

	GoalAdd.prototype.calculate = function () {
	  var text = this.quilljs_textarea_goaladd.getText();
	  if (this.options.unit === 'word') {
		text = text.trim();
		// Splitting empty text returns a non-empty array
		return text.length > 0 ? text.split(/\s+/).length : 0;
	...
	GoalAdd.prototype.update = function () {
	  var length = this.calculate();
	...
	Quill.register('modules/goaladd', GoalAdd);
	quilljs_textarea_goaladd('.quilljs-textarea-goaladd', {
	modules: {
		goaladd: { container: "#goal-add-counter", unit: 'word'},
		toolbar:[
		[{font:[]},{size:[]}],
		["bold","italic","underline","strike"],
		[{color:[]},{background:[]}],
