...
function addFundRecievedEntry() {	
	...
	// var myCheckboxes = [];
        // $('input[name="comitted[]"]').each(function() {
           // myCheckboxes.push($(this).val().replace(/,/g, ""));
        // });
	...
	for(i;i<=new_total_fund_recieved;i++){
		...
		// if((reciept == '' && hiddenReciept == '') || (reciept == '' && typeof(hiddenReciept) === "undefined")){ 
			// return false; 
		// }
	}
...
function getBalanceAmt(counter='')
{
	...
	if(counter != ''){
		...
		// if(comitted != '' && (comitted.replace(/,/g, "") >= totalDonationAmt || comitted.replace(/,/g, "") <= minDonationAmt)){
			// var msg = 'Comitted amount can not be greater than Total Project cost and not less than minumum donation amount.';
			// $.toast({
				// heading: '',
				// text: msg,
...
function RemoveTempGalleryImage(thislement,id) 
{
	if(id!='')
	{
		...
		$('#galleryImages').val('');
		var deleted_image_value = $('#deleted_image_ids').val();
		if(deleted_image_value == ''){
			$('#deleted_image_ids').val(id);
		}else{
			var new_value = deleted_image_value+','+id;
			$('#deleted_image_ids').val(new_value);
		}
	}
}
...
function addGoalEntry() { 
  ...
  for(i;i<=total_goal_recieved;i++){
    ...
    goalImg = $("#goal_"+i).val();
    hiddenGoalImage = $("#hiddenGoalImage_"+i).val();
