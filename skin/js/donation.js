$(document).ready(function () {
  ...
  function initDonationSlider(
    ...
    // Parse minimum and maximum donation amounts from hidden fields
    var min_donation = parseInt($(minHiddenId).val());
    var max_donation = parseInt($(maxHiddenId).val());

    // Initialize the slider with dynamic min, max, and values
    $(sliderId).slider({
      range: true,
      min: min_donation,
      max: max_donation,
      values: [min_donation, max_donation],
      slide: function (event, ui) {
        // Update display fields with formatted values as the slider moves
        $(minDisplayId).val(ui.values[0]);
        convertToINRFormat($(minDisplayId).val(), $(minDisplayId));
        $(maxDisplayId).val(ui.values[1]);
        convertToINRFormat($(maxDisplayId).val(), $(maxDisplayId));

        SearchFilterProjectsList();
      },
    });

    // Set initial display fields with formatted values
    $(minDisplayId).val($(sliderId).slider("values", 0));
    convertToINRFormat($(minDisplayId).val(), $(minDisplayId));

    $(maxDisplayId).val($(sliderId).slider("values", 1));
    convertToINRFormat($(maxDisplayId).val(), $(maxDisplayId));
  }

  // Initialize each slider with respective IDs
  initDonationSlider(
    "#donation_slider_range",
    "#donation_min_amount_hide",
    "#donation_max_amount_hide",
    "#donation_min_amount",
    "#donation_max_amount"
  );
  ...
  initDonationSlider(
    "#another_slider_range",
    "#another_min_amount_hide",
    "#another_max_amount_hide",
    "#another_min_amount",
    "#another_max_amount"
  );
  ...
  $("#donation_slider_range").slider({
    ...
    slide: function (event, ui) {
      ...
      convertToINRFormat(
        $("#donation_min_amount").val(),
        $("#donation_min_amount")
      );
      $("#donation_max_amount").val(ui.values[1]);
      convertToINRFormat(
        $("#donation_max_amount").val(),
        $("#donation_max_amount")
      );
...
function SearchFilterProjectsList() {
  ...
  $.ajax({
    type: "POST",
    dataType: "html",
    url: BASE_URL + "donation/ListProjects/",
    data: {
      search: search,
      pt_project: pt_project,
      pt_program: pt_program,
      ft_onetime: ft_onetime,
      ft_instpay: ft_instpay,
      ft_milestone: ft_milestone,
      damt_1: damt_1,
      damt_2: damt_2,
      damt_3: damt_3,
      damt_4: damt_4,
      search_by_loc: search_by_loc,
      csr_num: csr_num,
...
function loadmoreProjects() {
  ...
  $.ajax({
    type: "POST",
    dataType: "html",
    url: BASE_URL + "donation/SeeMoreProjects/",
    data: {
      search: search,
      pt_project: pt_project,
      pt_program: pt_program,
...
function clearfilters(flag, field_id = false, field_val = false) {
  if (flag == 1) {
    ...
    $("#donation_min_amount").val(
      $("#donation_slider_range").slider("values", 0)
    );
    convertToINRFormat(
      $("#donation_min_amount").val(),
      $("#donation_min_amount")
    ...
    $("#donation_max_amount").val(
      $("#donation_slider_range").slider("values", 1)
    );
    convertToINRFormat(
      $("#donation_max_amount").val(),
      $("#donation_max_amount")
    ...
    $("option", $("#searchException")).each(function (element) {
      $(this).removeAttr("selected").prop("selected", false);
    });
    $("#searchException").multiselect("reload");
    ...
    if (field_id == "pt_project") {
      ...
      var beneficiary_option = $(
        '#searchBeneficiary option[value="' + field_val + '"]'
      );
      beneficiary_option.prop("selected", false);
      $("#searchBeneficiary").multiselect("reload");
    } else if (field_id == "search_exception") {
      var exception_option = $(
        '#searchException option[value="' + field_val + '"]'
      );
      exception_option.prop("selected", false);
      $("#searchException").multiselect("reload");
    }
...
function filterSectorProjectsList() {
  ...
  $.ajax({
    type: "POST",
    dataType: "html",
    url: BASE_URL + "donation/filterSectorProjectsList/",
